...
def extract(zip_path: Path, into: Path) -> dict[str, Path]:
    """CSV members only, flattened by basename."""
    out = {}
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if args.prior:
            ...
            added_files = sorted(set(current) - set(prior))
            removed_files = sorted(set(prior) - set(current))
            for f in added_files:
                schema_changes.append(f"file only in current batch: {f}")
            for f in removed_files:
                schema_changes.append(f"file only in prior batch: {f}")
            common = sorted(set(prior) & set(current))
            for f in common:
                old = header_of(prior[f])
                new = header_of(current[f])
                if old != new:
                    ...
                    schema_changes.append(f"{f}: " + "; ".join(detail))
            if schema_changes:
            ...
            for f in common:
                old_h = header_of(prior[f])
                new_h = header_of(current[f])
                shared = [c for c in old_h if c in new_h]
                if not shared:
                    print(f"  {f}: no columns in common, rows not comparable")
                    continue
                p_sel = f"SELECT {norm_cols(shared)} FROM {csv_rel(prior[f])}"
                c_sel = f"SELECT {norm_cols(shared)} FROM {csv_rel(current[f])}"
                only_p = con.execute(
                ...
                total_c = con.execute(
                    f"SELECT count(*) FROM {csv_rel(current[f])}"
                ).fetchone()[0]
                note = "" if old_h == new_h else " (compared on shared columns only)"
                print(f"  {f}: only in prior: {only_p} · only in current: {only_c} · current total: {total_c}{note}")
                for label, sql, n in (
